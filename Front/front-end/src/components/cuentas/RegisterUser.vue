<template>
  <div class="register-div">
    <form v-if="!showConfirmationModal" @submit.prevent="submitForm"
      class="form-background px-36 py-10 rounded-xl shadow-md w-7/10">
      <div class="mb-3">
        <label for="email" class="block title-style font-bold mb-2 text-center">Registrar Cuenta</label>
      ...
      <div class="mb-3">
        ...
        <input type="text" v-model="username" @blur="validateUsername" id="username" name="username"
          class="shadow appearance-none border rounded-xl h-16 w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
      ...
      <div class="mb-3">
        ...
        <div class="flex items-center">
          ...
          <button type="button"
            ...
            @mousedown="showPassword('password')" @mouseup="hidePassword('password')"
            @mouseleave="hidePassword('password')">
      ...
      <div class="mb-8">
        ...
        <div class="flex items-center">
          <input :type="confirmPasswordFieldType" v-model="confirmPassword" @blur="validateConfirmPassword"
            id="confirm-password" name="confirm-password"
            class="shadow appearance-none border rounded-xl h-16 w-full py-2 px-3 text-gray-700 mr-5 leading-tight focus:outline-none focus:shadow-outline">
          <button type="button"
            class="w-16 h-16 rounded-xl ml-auto button-color flex items-center justify-center active:bg-blue-900"
            @mousedown="showPassword('confirmPassword')" @mouseup="hidePassword('confirmPassword')"
      ...
      <div class="flex justify-center">
        <button type="submit"
          class="button-color hover:bg-blue-900 text-style font-bold py-8 px-12 rounded-xl w-auto focus:outline-none focus:shadow-outline">Registrar
          Cuenta</button>
      </div>
      <div class="flex justify-center items-center mt-4">
        <p class="text-question">¿Ya tiene cuenta?</p>
        <router-link to="/login" class="ml-2 text-indigo-900 text-lg hover:text-indigo-600">Inicie sesión
          aquí</router-link>
      </div>
    </form>
    <div v-if="showConfirmationModal" class="modal">
      <div class="modal-content">
        <p>Se ha enviado un correo de confirmación. ¡Revise su bandeja de entrada!</p>
        <button @click="cuentas / Login.vue" type="submit" class="hover:bg-blue-900">Aceptar</button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    validateUsername() {
      this.usernameError = this.username ? "" : "El nombre de usuario es requerido.";
    },
    validatePassword() {
    ...
    submitForm() {
      ...
      if (!this.emailError && !this.usernameError && !this.passwordError && !this.confirmPasswordError) {
